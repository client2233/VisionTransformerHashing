...
def train_val(config, bit):
    ...
    for epoch in range(start_epoch, config["epoch"] + 1):
        ...
        print(
            "%s-%s[%2d/%2d][%s] bit:%d, dataset:%s, training...."
            % (
                config["info"],
                config["net_print"],
                epoch,
        ...
        for image, label, ind in train_loader:
            ...
            # 在现有 loss 后添加正交正则
            # hash_layer = Sequential(Dropout, Linear(→1024), ReLU, Linear(1024→hash_bit))
            W = net.hash_layer[3].weight  # 形状 [hash_bit, 1024]
            reg = (W @ W.T - torch.eye(hash_bit, device=device)).pow(2).sum()
            loss += config.get("ortho_lambda", 0.01) * reg
        ...
        if (epoch) % config["test_map"] == 0:
            ...
            print(
                "%s epoch:%d, bit:%d, dataset:%s, MAP:%.3f, Best MAP: %.3f"
                % (config["info"], epoch, bit, config["dataset"], mAP, Best_mAP)
            )
            f.write(
                "Test | Epoch %d | MAP: %.3f | Best MAP: %.3f\n"
                % (epoch, mAP, Best_mAP)
            )
            print(config)
...
class DSHLoss(torch.nn.Module):
    def __init__(self, config, bit):
        ...
        self.Y = (
            torch.zeros(config["num_train"], config["n_class"])
            .float()
            .to(config["device"])
        )
