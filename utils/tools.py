...
def cifar_dataset(config):
    ...
    train_dataset = MyCIFAR10(
        root=cifar_dataset_root, train=True, transform=transform, download=True
    )

    test_dataset = MyCIFAR10(root=cifar_dataset_root, train=False, transform=transform)

    database_dataset = MyCIFAR10(
        root=cifar_dataset_root, train=False, transform=transform
    )
    ...
    database_loader = torch.utils.data.DataLoader(
        dataset=database_dataset, batch_size=batch_size, shuffle=False, num_workers=4
    )
    ...
    for data_set in ["train_set", "test", "database"]:
        dsets[data_set] = ImageList(
            ...
            transform=image_transform(
                config["resize_size"], config["crop_size"], data_set
            ),
        )
